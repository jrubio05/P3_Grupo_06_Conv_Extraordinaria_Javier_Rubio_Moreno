...
PxRigidDynamic* PhysxManager::createDynamic(const PxTransform& transform, const PxVec3& velocity) {
	PxRigidDynamic* dynamic = PhysxManager::instance()->getPhysX()->createRigidDynamic(transform);
	dynamic->setLinearVelocity(velocity);
	mScene->addActor(*dynamic);
	return dynamic;
}

PxRigidDynamic* PhysxManager::createDynamic(const PxTransform& transform, const PxGeometry& geometry,
		PxMaterial& material, const PxVec3& velocity) {
...
PxRigidDynamic* PhysxManager::createDynamic(const PxTransform& transform, PxShape* shape,
...
PxRigidStatic* PhysxManager::createStaticRigid(const PxTransform& transform) {
	PxRigidStatic* body = PhysxManager::instance()->getPhysX()->createRigidStatic(transform);
	mScene->addActor(*body);
	return body;
}

PxRigidStatic* PhysxManager::createStaticRigid(const PxTransform& transform,
		const PxGeometry& geom, PxMaterial& material) {
